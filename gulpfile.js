...
async function setVersion() {
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
            gulp.src('./CarryOnLib.csproj')
                .pipe(replace(/<Version>.*?<\/Version>/, `<Version>${modVersion}</Version>`))
                .pipe(gulp.dest('./'))
                .on('end', () => {
                    console.log('Version updated successfully in CarryOnLib.csproj');
                    resolve();
                })
                .on('error', reject);
        });
...
function renameZip() {
    ...
    const oldName = `CarryOnLib.zip`;
    const newName = `CarryOnLib-${vintageStoryVersion}_v${modVersion}.zip`;
